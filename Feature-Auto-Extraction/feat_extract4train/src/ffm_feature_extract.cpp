...
int GetFieldID(string filename, string featurename, FEATURE_TYPE_E type){
    ...
    if(fname.length() > 0){
        ...
        if(it == g_mapFields.end()){
            pthread_mutex_lock(pmutex);
            g_mapFields[fname]=g_mapFields.size();
            int id = g_mapFields[fname];
            if(debug_mode >= 2){
                cout << fname <<":" << id << endl;
            }
            pthread_mutex_unlock(pmutex);
...
int GetFeatureid(string filename, string featurename, string strvalue, FEATURE_TYPE_E type){
    ...
    if(fname.length() > 0){
        ...
        if(it == g_mapFeatures.end()){
            pthread_mutex_lock(pmutex);
            g_mapFeatures[fname]=g_mapFeatures.size();
            int id = g_mapFeatures[fname];
...
bool GetMergeFIDs(int &mfieldid,
    ...
    if(hasMF && mkindex > -1 && mvindex > -1){
        ...
        if(itfld != g_mapFields.end()){
            ...
            pthread_mutex_lock(pmutex);
            g_mapFields[fldname] = g_mapFields.size();
            mfieldid = g_mapFields[fldname];
            pthread_mutex_unlock(pmutex);
        }
        ...
        if(itfeat != g_mapFeatures.end()){
            ...
            pthread_mutex_lock(pmutex);
            g_mapFeatures[featname] = g_mapFeatures.size();
            mfeatureid = g_mapFeatures[featname];
...
int main(int argc, char **argv) {
    ...
    int i;
    if (argc == 1) {
        printf("ffm feature extract toolkit v 1.0\n\n");
        printf("Options:\n");
        printf("Parameters:\n");
        printf("\t-in <path>\n");
        ...
        printf("\nExamples:\n");
        printf("./ffm_feature_extract -in ./ -suffix txt -out out/ffm_features.txt -debug 2\n\n");
        return 0;
    }
    in_path[0] = 0;
    out_file[0] = 0;
