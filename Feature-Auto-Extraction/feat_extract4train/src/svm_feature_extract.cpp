...
char in_path[MAX_STRING], suffix[MAX_STRING], out_file[MAX_STRING];
int debug_mode = 2;
...
int GetFid(string filename, string featurename, string strvalue, FEATURE_TYPE_E type){
    ...
    switch(type){
        ...
        case SCD_E://special format <number + string>, select the number and /100
            ...
            break;
        case CD10_E://age
            fname = filename+featurename+"_NULL";
            if(strvalue != "NULL"){
    ...
    if(fname.length() > 0){
        ...
        if(it == g_mapFids.end()){
            pthread_mutex_lock(pmutex);
            g_mapFids[fname]=g_mapFids.size();
            int id = g_mapFids[fname];
            pthread_mutex_unlock(pmutex);
            return id;
        }else{
            return it->second;
...
bool GetMergeFIDs(int &mfeatureid,
    ...
    if(hasMF && mkindex > -1 && mvindex > -1){
        ...
        if(itfeat != g_mapFids.end()){
            ...
            pthread_mutex_lock(pmutex);
            g_mapFids[featname] = g_mapFids.size();
            mfeatureid = g_mapFids[featname];

            if(debug_mode >= 2){
                cout << featname <<":" << mfeatureid << endl;
            }
            pthread_mutex_unlock(pmutex);
        }
...
bool AddFeature(int fid, int uid, int val, bool isCVal = false){
    if(fid < 0 || uid < 0 || val < 0) return false;
...
void ExtractModel(FILE * fp){
    ...
    for(auto &it:vUids){
        ...
        if(iter != g_mapResFeatures.end()){
            ...
            sort(vFids.begin(), vFids.end(), cmpIds);
            if(debug_mode >= 2){
...
int main(int argc, char **argv) {
    ...
    int i;
    if (argc == 1) {
        printf("libsvm(xgboost) feature extract toolkit v 1.0\n\n");
        printf("Options:\n");
        printf("Parameters:\n");
        printf("\t-in <path>\n");
        printf("\t\the input <path> of confgure and data files, make sure there are 'conf' and 'data' folders in this <path>");
        printf("\t-suffix <string>\n");
        printf("\t\the suffix of input data files, eg:css/txt");
        printf("\t-out <file>\n");
        printf("\t\twill output two files: featureid:value file and featureid list file\n");
        printf("\t-debug <int>\n");
        printf("\t\tSet the debug mode (default = 2 = more info during training)\n");
        printf("\nExamples:\n");
        printf("./svm_feature_extract -in ./ -suffix txt -out out/svm_features.txt -debug 2\n\n");
        return 0;
    }
    in_path[0] = 0;
    out_file[0] = 0;
